...
const HomeItem = (props) => {
  const deleteFolder = () => {
    try {
      props.selectedItem.includes(props.item.folderId)
        ? props.selectedItem.splice(
            props.selectedItem.indexOf(props.item.folderId),
            1
          )
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => {
        props.screenType == "HomeScreen" && !props.isDelete
          ? switchPrivate()
          : props.screenType !== "AddItemScreen" && props.isDelete
          ? deleteFolder()
          : saveText();
      }}
    >
